  ...
  Async.waterfall([
    function (callback) {
      if (payload && payload.userName && payload.password) {
        ...
        return callback(new Error('Username and password incorrect'));
      }
    },
...
exports.saveUser = function (request, reply) {
  ...
  Async.waterfall([
    ...
    function (password, userId, callback) {
      client.hmset(`user:${userId}`, 'userName', payload.userName, 'fullName', payload.fullName, 'password', password, 'email', payload.email, 'class', payload.class, 'year', payload.year, (err) => {
        if (err) {
          return callback(err);
        }
        return callback(null, userId);
