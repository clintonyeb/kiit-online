...
exports.previewHandler = function (request, reply) {
  const path = Path.resolve(`${__dirname}/assets/previews/${request.params.filename}`);
  return reply.file(path);
};

exports.uploadsHandler = function (request, reply) {
  const path = Path.resolve(`${__dirname}/uploads/${request.params.filename}`);
  return reply.file(path);
};

exports.avatarsHandler = function (request, reply) {
  const path = Path.resolve(`${__dirname}/assets/${request.params.path}/${request.params.filename}`);
  return reply.file(path);
};
...
exports.saveUser = function saveUser(request, reply) {
  ...
  Async.waterfall([
    ...
    function (password, callback) {
      client.hmset(`user:${userName}`, 'userName', userName, 'fullName', payload.fullName,
        'password', password, 'email', payload.email, 'class', payload.class, 'year', payload.year, 'avatar', 'boy.svg', (err) => {
          if (err) {
            return callback(err);
          }
...
/* exports.recoverPass = (request, reply) => {
  const payload = request.payload;
  const client = request.server.app.redisClient;

  client.hget(`user:${payload.userName}`, '')
};*/
