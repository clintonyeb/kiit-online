...
exports.avatarsHandler = function (request, reply) {
  const path = Path.resolve(`${__dirname}/assets/avatars/${request.params.filename}`);
  return reply.file(path);
};
...
exports.saveUser = function saveUser(request, reply) {
  ...
  Async.waterfall([
    ...
    function (password, callback) {
      client.hmset(`user:${userName}`, 'userName', userName, 'fullName', payload.fullName, 'password', password, 'email', payload.email, 'class', payload.class, 'year', payload.year, 'avatar', 'boy.svg', (err) => {
        if (err) {
          return callback(err);
        }
