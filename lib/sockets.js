...
module.exports = function main(redisClient) {
  ...
  let socketHandler = function socketHandler(socket) {
    ...
    socket.on('getSlides', (data) => {
      Async.waterfall([
        function getUserId(callback) {
          redisClient.hget('users', data.userName, (err, userId) => callback(err, userId));
        },
        function getUserClass(userId, callback) {
          redisClient.hget(`user:${userId}`, 'class', (err, userClass) => callback(err, userId, userClass));
        },
        function getIds(userId, userClass, callback) {
    ...
    socket.on('addSlide', (data) => {
      ...
      Async.waterfall([
        ...
        function getPostId(userId, userClass, callback) {
    ...
    socket.on('chat', (data) => {
      ...
      Async.waterfall([
        ...
        function getUserId(chatId, callback) {
          redisClient.hget('users', data.userName, (err, userId) => callback(err, chatId, userId));
        },
        function getUserClass(chatId, userId, callback) {
          redisClient.hget(`user:${userId}`, 'class', (err, userClass) => callback(err, chatId, userClass));
        },
    ...
    socket.on('getChats', (data) => {
      Async.waterfall([
        ...
        function getUserClass(userId, callback) {
          redisClient.hget(`user:${userId}`, 'class', (err, userClass) => callback(err, userClass));
        },
        ...
        return _.reverse(chatIds).map(id => redisClient.hgetall(`chat:${id}`, (err, chat) => {
          redisClient.hget('users', chat.userName, (err, userId) => {
            redisClient.hmget(`user:${userId}`, 'userName', 'fullName', 'class', 'avatar', (err, docs) => {
              const res = chat;
              res.id = id;
              const user = {
                userName: docs[0],
